...
export async function getStorefrontSnapshot(options = {}) {
  ...
  const productsQuery = client
    .from("products")
    .select("id, name, slug, short_description, description, base_price, compare_at_price, currency, featured, category, seo_title, seo_description, brands(name, slug)")
    .eq("status", "active")
  ...
  const allProducts = (productsData ?? []).map(normalizeProduct);
