...
import { FolderKanban, Package2, Tags } from "lucide-react";
  ...
  return (
    <div className="grid gap-6 xl:gap-8">
        ...
        <div className="relative grid gap-6 xl:grid-cols-[1.1fr_0.9fr] xl:items-start">
          <div>
            ...
            <h1 className="mt-5 max-w-3xl text-4xl font-semibold tracking-tight text-white sm:text-5xl">
              Keep products and categories easy to review from one premium surface.
            </h1>
          ...
          <article className="rounded-[1.75rem] border border-white/10 bg-white/8 p-5 backdrop-blur-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-sky-200">Catalog snapshot</p>
                <p className="mt-1 text-base font-semibold text-white">{catalog.productCount} visible products across {catalog.categoryCount} live categories.</p>
              </div>
            </div>
          </article>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        {links.map((item) => (
          <Link key={item.href} href={item.href} className="rounded-4xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl transition hover:border-sky-300/25 hover:bg-white/8">
